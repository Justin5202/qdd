...
/* 时间戳转换 */
Utils.myFormatDate = function (time) {
...
Utils.generateGroupSysMsg = function (data) {
  let text
  switch (data.attach.type) {
    case 'addTeamMembers':
      text = `欢迎"${data.attach.users[0].nick}"进入该房间`
      break
    case 'leaveTeam':
      text = `"${data.attach.users[0].nick}"离开了该房间`
      break
    case 'removeTeamMembers':
      text = `"${data.attach.users[0].nick}"被管理员踢出房间`
      break
    case 'addTeamManagers':
      text = `"${data.attach.users[0].nick}"被任命管理员身份`
      break
    case 'removeTeamManagers':
      text = `"${data.attach.users[0].nick}"被解除管理员身份`
      break
    // case 'xxx':
    // 直播公告已更新
    // break;
