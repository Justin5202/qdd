...
export function onBroadcastMsg(msg) {
...
function countDown (type) {
  ...
  let timer = setInterval(function() {
    if(time == 0) {
      if(store.state.whoAddMsg.length == 0) {
        clearInterval(timer)
        window.history.go(-1)
        store.commit('updateSysMsg', '')
      } else {
        clearInterval(timer)
      }
    } else {
...
export function onMsg (msg) {
  console.log(msg)
  let obj = {} //进群的信息：时间、账号、ID
  msg = formatMsg(msg)
  store.commit('putMsg', msg)
  if(msg.content) {
    if(JSON.parse(msg.content).type == 'invite'){
      if(msg.from === store.state.myInfo.acid) {
        Toast('邀请成功')
      }
      if(msg.to === store.state.myInfo.acid) {
        let obj = {
            from: msg.fromNick,
            to: msg.to,
            room: JSON.parse(msg.content).room
        }
        store.commit('updateInviteInfo', obj)
        if(/chat/.test(router.app._route.path)) {
          return
        }
        store.commit('updateInviteShow', true)
      }
    } else if(JSON.parse(msg.content).type == 'sys_msg') {
      store.commit('updateSysMsg', msg)
    } 
  }
  if(msg.attach && msg.attach.type == "addTeamMembers") {
    obj.time = msg.time
    obj.account = msg.attach.accounts[0]
    obj.teamId = msg.to
    // countDown(msg.attach.type)
  } else if(msg.attach && msg.attach.type == "leaveTeam") {
    // countDown(msg.attach.type)
  } 
  if(obj && msg.time - obj.time < 2 * 60 * 1000 && msg.time - obj.time != 0) {
    if(msg.to == obj.teamId && obj.account == msg.from) {
      store.commit('updateWhoAddMsg', msg)
    } 
  }
